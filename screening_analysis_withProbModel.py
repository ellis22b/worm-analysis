...
'''
Usage example:
./screening_analysis_withProbModel.py --toAssess test_files/kw9_export.txt test_files/kw11_export.txt test_files/kw12_export.txt --expNames kw9 kw11 kw12 --strain N2 --lifestage L4 --drug ALB --concUnits 0 --representative 3 --molarmass 265.333
'''
...
def analysis_structure(args_object, class_object, impute=False): ###go back in and fill out fignames based on impute
    if args_object.plotLine3 or args_object.plotLine1:
       class_object.driveLinePlots(args_object.plotLine3, args_object.plotLine1, args_object.isep, args_object.expNames, impute)
    if args_object.plotIT50 or args_object.plotLT50:
       class_object.driveSurvivalTimePlots(args_object.plotIT50, args_object.plotLT50, args_object.rep, args_object.expNames, impute)
    if args_object.plotIC50 or args_object.plotLC50:
        class_object.driveIC(args_object.plotIC50, args_object.plotLC50, args_object.C_day, args_object.x0_val, args_object.hill1, args_object.hill3, impute)
    class_object.reportTable(args_object.expNames[args_object.rep-1], args_object.reportNum, args_object.plotIT50, args_object.plotLT50, args_object.plotIC50, args_object.plotLC50, impute)
...
def main():
    ...
    #this block is just here while I'm testing, will remove later
    # PWM_params = analysis_instance.learn_PWM_params()
    # target_nums = analysis_instance.get_num_treated()
    # sequences_aggregated, sequences_split = analysis_instance.generate_sequences(PWM_params, target_nums, args.seed)
    # analysis_instance.recreate_scores3(sequences_aggregated, sequences_split) #need to remake self.scores3_by_conc and self.scores3_by_well


    analysis_structure(args, analysis_instance)
    ...
    # if not analysis_instance.get_consistent_num():
    #     PWM_params = analysis_instance.learn_PWM_params()
    #     target_nums = analysis_instance.get_num_treated()
    #     sequences_aggregated, sequences_split = analysis_instance.generate_sequences(PWM_params, target_nums, args.seed)
    #     analysis_instance.recreate_scores3(sequences_aggregated, sequences_split) #need to remake self.scores3_by_conc and self.scores3_by_well
    #     #Rerun self.find_motility_index_score() and self.find_mortality_score()
    #     analysis_instance.find_mortality_score()
    #     analysis_instance.find_motility_index_score()
    #     analysis_structure(args, analysis_instance, impute=True)
...
def generate_parser():
    ...
    parser.add_argument('--seed', action='store', dest='seed', type=int, default=36, required=False, help='the seed to use with numpy.random.choice')
    return parser
...
class WormAnalysis():
        ...
        for i, file in enumerate(toAssess):
            ...
            if i == 0:
                ...
                for well, conc in zip(np.array(df.loc[:, 'Well']).astype(np.int32)[0:self.num_concentrations*3], np.array(df.loc[:,'Concentration'])[0:self.num_concentrations*3]):
                    ...
                    if float(conc) not in self.conc_to_well_index:
                        self.conc_to_well_index[float(conc)] = []
                    self.conc_to_well_index[float(conc)].append(well_index)
    ...
    def get_consistent_num(self):
        return self.consistent_num
    ...
    def find_numTotal(self):
        if self.consistent_num:
            '''Note when consistent_num, the following 3 are equivalennt:
            np.sum(self.scores3_by_conc[:,:,0,:].reshape(-1, 4*self.num_experiments), axis=1)
            np.sum(np.amax(np.sum(self.scores3_by_conc, axis=1), axis=1), axis=1)
            np.sum(np.sum(np.amax(np.sum(self.scores3_by_well, axis=1), axis=1), axis=1).reshape((self.num_concentrations, 3)), axis=1)
            '''
            total_nums = np.sum(self.scores3_by_conc[:,:,0,:].reshape(-1, 4*self.num_experiments), axis=1)
        else:
            '''Note when not consistent_num, of the 3 terms in the earlier note, the last two are not always equivalent with the former underestimating, making the final by well form the better one'''
            logging.info("Note, since there were not a consistent number of worms in a given well for every day of the experiment, the total number of nematodes treated by"
    ...
    def learn_PWM_params(self):
        ...
        params = np.zeros((self.num_concentrations, 4, self.num_days+1))
    ...
    def find_motility_index_score(self):
        ...
        #Note for these here whether self.consistent_num or not, the number total is dependent on the number of worms seen on each day rather than the minimum number that should have been seen
        ...
        divided = adjusted_sum/np.sum(self.scores3_by_conc, axis=1) #divide the sum above (adjusted_sum) by the number of worms total in the well (the denominator); this is a day-specific totals
    ...
    def find_mortality_score(self):
        ...
        divided_to_percent = adjusted_sum/np.sum(self.scores3_by_conc, axis=1)*100 #divide the sum above (adjusted_sum) by the number of worms total in the well (the denominator); again this is a day-specific totals, change to percentage
    ...
    def driveLinePlots(self, plotLine3, plotLine1, isep, expNames, impute=False):
    ...
    def findSurvivability(self, toPopulate, toAnalyze, rep_index, expName, motility=False, mortality=False):
        ...
        if self.consistent_num:
            num_total = np.sum(toAnalyze_expSpec[:,:,0], axis=1).reshape((self.num_concentrations, 1)) #this num total assumes the numbers are consistent each day

        else:
            num_total = np.sum(toAnalyze_expSpec, axis=1) #this num_total is representative of the number seen each day rather than the true minimum number that should be there
    ...
    def driveSurvivalTimePlots(self, plotIT50, plotLT50, representative, expNames, impute=False):
        if representative == 0:
    ...
    def driveIC(self, plotIC50, plotLC50, C_day, x0_val, hill1, hill3, impute=False):
        ...
        #Use this to go from well to conc etc
        #self.well_index_to_conc = {}
        self.C_day = C_day
        self.x0_val = x0_val
        totals = np.sum(self.scores3_by_well[:,:,self.C_day-1,:], axis=1)
        #here whether self.consistent_num or not, the number total is dependent on the number of worms seen that day rather than the minimum number that should have been seen
    ...
    def reportTable(self, rep_exp, reportNum, plotIT50, plotLT50, plotIC50, plotLC50, impute=False):
